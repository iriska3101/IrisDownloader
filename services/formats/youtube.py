...
def _provider_is_ready() -> bool:
    return (
        ...
        and (BGUTIL_SERVER / "build" / "generate_once.js").exists()
    )
...
def _prepare_bgutil_provider() -> str | None:
    ...
    if _provider_is_ready():
        print("YOUTUBE POT: BgUtils provider уже готов", flush=True)
    ...
    with _PROVIDER_LOCK:
        ...
        try:
            ...
            print("YOUTUBE POT: устанавливаю BgUtils provider", flush=True)
            ...
            if clone.stderr.strip():
                print(
                    "YOUTUBE POT: git: " + clone.stderr.strip()[-600:],
                    flush=True,
                )
            ...
            if npm_result.stderr.strip():
                print(
                    "YOUTUBE POT: npm: " + npm_result.stderr.strip()[-800:],
                    flush=True,
                )
            ...
            if tsc_result.stderr.strip():
                print(
                    "YOUTUBE POT: tsc: " + tsc_result.stderr.strip()[-1200:],
                    flush=True,
                )
            ...
            if stderr:
                print("YOUTUBE POT BUILD STDERR: " + stderr[-2000:], flush=True)
            if stdout:
                print("YOUTUBE POT BUILD STDOUT: " + stdout[-1200:], flush=True)
            shutil.rmtree(BGUTIL_ROOT, ignore_errors=True)
            return None
        ...
        if not BGUTIL_BUILT_FILE.exists():
            print("YOUTUBE POT: build/main.js не создан", flush=True)
            shutil.rmtree(BGUTIL_ROOT, ignore_errors=True)
            return None

        BGUTIL_READY_MARKER.write_text("ok\n", encoding="utf-8")
        print("YOUTUBE POT: BgUtils provider готов", flush=True)
        return str(BGUTIL_SERVER)


def warm_youtube_provider() -> bool:
    """Фоновый прогрев provider после deploy, до первого запроса пользователя."""
    print("YOUTUBE POT WARMUP: начинаю фоновую подготовку", flush=True)
    provider = _prepare_bgutil_provider()
    if provider:
        print("YOUTUBE POT WARMUP: готов", flush=True)
        return True
    print("YOUTUBE POT WARMUP: не удалось подготовить", flush=True)
    return False
...
def get_youtube_options() -> dict[str, Any]:
    ...
    if provider_home:
        script_path = Path(provider_home) / "build" / "generate_once.js"
        if script_path.exists():
            extractor_args["youtubepot-bgutilscript"] = {
                "script_path": [str(script_path)],
            }
            print("YOUTUBE POT: script provider подключён", flush=True)
        else:
            print("YOUTUBE POT: generate_once.js не найден", flush=True)

    return {
        "format": "bv*[ext=mp4]+ba[ext=m4a]/b[ext=mp4]/bv*+ba/b",
        "format_sort": ["res", "fps", "hasaud"],
